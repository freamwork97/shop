...
public class CartController extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		ArrayList<CartVO> arrCart = session.getAttribute("arrCart") == null ? new ArrayList<CartVO>()
				: (ArrayList<CartVO>) session.getAttribute("arrCart");
		...
		case "/cart/insert": // 장바구니 상품넣기
			...
			for (CartVO vo : arrCart) {
				if (gid.equals(vo.getGid())) {
					vo.setQnt(vo.getQnt() + 1); // 장바구니에 존재하면 수량1증가
					find = true;
				}
			}
			...
			if (!find) {
				...
				cvo.setQnt(1);
				arrCart.add(cvo); // 장바구니에 없으면 장바구니에 추가
			}
		...
		case "/cart/delete":
			...
			for (CartVO vo : arrCart) {
				if (gid.equals(vo.getGid())) {
					arrCart.remove(vo); // 카드에서 상품을 찾으면 삭제
					break;
				}
			}
			session.setAttribute("arrCart", arrCart);
