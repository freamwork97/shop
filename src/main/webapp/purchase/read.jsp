...
<div class="row my-5">
  <div class="col">
    <h1 class="text-center mb-5">구매정보</h1>
    <div class="card p-3">
      <div class="row mb-3">
        <div class="col">주문번호: ${vo.pid}</div>
        <div class="col">주문자: ${vo.uname} ${vo.uid}</div>
        <div class="col">전화번호: ${vo.phone}</div>
        <div class="col">주문일자: ${vo.purDate}</div>
      </div>
      <hr />
      <div class="row mb-3">
        ...
        <div class="col">
          주문총금액:
          <fmt:formatNumber value="${vo.purSum}" pattern="#,###원" />
        </div>
        <div class="col">주문상태:
            <c:out value="${vo.status==0 ?'결제대기중' :''}"/>
            <c:out value="${vo.status==1 ?'결제완료' :''}"/>
            <c:out value="${vo.status==2 ?'배송준비중' :''}"/>
